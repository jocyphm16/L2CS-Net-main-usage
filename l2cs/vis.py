...
def draw_gaze(a,b,c,d,image_in, pitchyaw, thickness=2, color=(255, 255, 0),sclae=2.0):
    ...
    # --- NEW CODE: PROJECT VIEWPOINT DOT ---
    # We multiply dx and dy by 5 to stretch the line out further like a laser pointer
    distance_multiplier = 5.0 
    
    target_x = int(pos[0] + dx * distance_multiplier)
    target_y = int(pos[1] + dy * distance_multiplier)
    
    # Draw a filled red circle at the target coordinate (BGR color format)
    cv2.circle(image_out, (target_x, target_y), radius=12, color=(0, 0, 255), thickness=-1)
    return image_out
